...
const radius = (min = 0.75, max = 1.25) => round(min + Math.random() * (max - min));

// Configuration: Base size and relative sizes for 3 circles
const BASE_SIZE = 75; // pixels
const RELATIVE_SIZES = [radius(0.85, 1.1), radius(0.55, 0.85), radius(0.75, 0.95)]; // percentages of base size
...
function generateCircleData(width, height) {
    ...
    return circles.map((circle) => ({
        cx: Math.round(circle.cx), cy: Math.round(circle.cy), r: circle.r
    }));
}
 ...
 */
function metaball(radius1, radius2, center1, center2, handleSize = HANDLE_SIZE, v = V) {
...
function metaballToPath(data) {
    ...
    return [
        'M', p1.map(round).join(','),
        'C', h1.map(round).join(','), h3.map(round).join(','), p3.map(round).join(','),
        'A', round(radius), round(radius), 0, escaped ? 1 : 0, 0, p4.map(round).join(','),
        'C', h4.map(round).join(','), h2.map(round).join(','), p2.map(round).join(','),
    ].join(' ');
}
